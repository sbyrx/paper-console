...
def test_receive_print_webhook_can_print_when_channel_inactive(monkeypatch):
...
def test_receive_print_webhook_returns_423_when_printer_busy(monkeypatch):
    ...
    monkeypatch.setattr(main_module.dial, "read_position", lambda: 1)
    monkeypatch.setattr(main_module, "try_begin_print_job", lambda debounce=False: False)
